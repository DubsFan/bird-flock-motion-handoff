...
import { BUILTIN_BIRD_TEMPLATE, CURATED_BIRD_TEMPLATES, type BirdTemplate, type SceneSource } from "@/lib/flock/types"
...
export function AssetPanel({ scene, flockName, birdTemplate, onScene, onBirdTemplate }: {
  scene: SceneSource
  flockName: string
    ...
    <section className="flex flex-col gap-5 p-4">
      ...
      <div>
        ...
        <p className="mt-1 text-xs leading-relaxed text-muted-foreground">
          Assigned to <strong className="font-medium text-foreground">{flockName}</strong>. Best input: one bird&apos;s eight numbered transparent PNG poses, selected together in order.
        </p>
      </div>
      <label className="flex flex-col gap-1.5 text-xs text-muted-foreground">
        <span>Curated artist source</span>
        <select
          aria-label="Curated artist source"
          value={BUILTIN_ARTWORK_OPTIONS.some((option) => option.id === birdTemplate.id) ? birdTemplate.id : "custom"}
          onChange={(event) => {
            const next = BUILTIN_ARTWORK_OPTIONS.find((option) => option.id === event.target.value)
            if (next) onBirdTemplate(next)
          }}
          className="h-9 rounded-md border border-border bg-secondary px-2 text-xs text-foreground"
        >
          {!BUILTIN_ARTWORK_OPTIONS.some((option) => option.id === birdTemplate.id) && <option value="custom">Custom uploaded artwork</option>}
      ...
      <p className="text-[10px] leading-relaxed text-muted-foreground">Lowest friction: import one identity ZIP containing its manifest, flight, approach, perch, and launch folders. For multiple identities, create one Flock tab per bird and import one bundle into each. Eight flat flight PNGs still work; a single centered PNG remains a fallback.</p>
      ...
      >
        <FileDown className="h-3.5 w-3.5" />Download artwork-agent prompt (.md)
      </a>
    </section>
  )
}
