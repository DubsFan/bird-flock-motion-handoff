...
MASTER="$DIR/birds-graceful-natural-flight-prores4444.mov"
WEBM="$DIR/birds-graceful-natural-flight-transparent.webm"
PREVIEW="$DIR/birds-graceful-natural-flight-review.mp4"

ffmpeg -y -i "$MASTER" -vf "crop=1672:940:0:0" -an \
  -c:v libvpx-vp9 -pix_fmt yuva420p -crf 28 -b:v 0 -auto-alt-ref 0 "$WEBM"

ffmpeg -y -f lavfi -i "color=c=#1F2937:s=1672x941:r=24:d=10" -i "$MASTER" \
