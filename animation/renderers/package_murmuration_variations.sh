...
for NAME in murmuration_flash murmuration_split murmuration_ribbon; do
  MASTER="$DIR/${NAME}_prores4444.mov"
  WEBM="$DIR/${NAME}_transparent.webm"
  REVIEW="$DIR/${NAME}_review.mp4"
  ffmpeg -y -v error -i "$MASTER" -vf "crop=1672:940:0:0" -an \
  ...
  ffprobe -v error -select_streams v:0 -show_entries stream=codec_name,pix_fmt,width,height,avg_frame_rate,duration:stream_tags=alpha_mode \
    -of default=noprint_wrappers=1 "$WEBM" > "$WEBM.ffprobe.txt"
  echo "packaged $NAME"
done
