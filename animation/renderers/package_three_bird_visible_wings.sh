...
MASTER="$DIR/three_bird_visible_wings_prores4444.mov"
WEBM="$DIR/three_bird_visible_wings_transparent.webm"
REVIEW="$DIR/three_bird_visible_wings_review.mp4"

ffmpeg -y -i "$MASTER" -vf "crop=1672:940:0:0" -an \
  -c:v libvpx-vp9 -pix_fmt yuva420p -crf 28 -b:v 0 -auto-alt-ref 0 "$WEBM"

ffmpeg -y -f lavfi -i "color=c=white:s=1672x941:r=24:d=8" -i "$MASTER" \
