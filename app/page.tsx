    ...
    try {
      ...
      if (raw) {
        ...
        if (parsed?.sequences?.length) {
          setProject(parsed)
          setActiveId(parsed.sequences[0].id)
          setHydrated(true)
          return
        }
      }
      ...
      <main className="mx-auto flex w-full max-w-[1400px] flex-1 flex-col gap-4 p-4 lg:flex-row">
        ...
        <section className="flex min-w-0 flex-1 flex-col gap-4">
          <Toolbar
            ...
            hasBackdrop={!!project.backdropDataUrl}
            onUpload={onUpload}
            onClearBackdrop={() => setProject((p) => ({ ...p, backdropDataUrl: null }))}
        ...
        {/* Inspector column */}
        <aside className="flex w-full shrink-0 flex-col overflow-hidden rounded-lg border border-border bg-card lg:w-[340px]">
